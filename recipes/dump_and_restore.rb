namespace :db do  
  ...
  task :dump_and_restore, :roles => :db, :only => {:primary => true} do
    ...
    run "cd #{current_release} && RAILS_ENV=#{rails_env} FILE=#{file_env} bundle exec #{rake} db:backup"
    get "#{current_release}/db/dump.tar.gz", "db/dump.tar.gz"
    cmd = "bundle exec rake RAILS_ENV=#{ ENV['RAILS_ENV'] || "development" } FILE=#{file_env} db:restore"
    puts cmd
    system cmd
  end
end
